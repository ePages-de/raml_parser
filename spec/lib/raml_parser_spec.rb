...
RSpec.describe RamlParser::Parser do
  ...
  it 'parses query parameters' do
    ...
    expect(raml.resources[0].methods['get'].query_parameters.map { |name,param| param.name }).to eq ['q1']
    expect(raml.resources[1].methods['get'].query_parameters.map { |name,param| param.name }).to eq ['q2']
